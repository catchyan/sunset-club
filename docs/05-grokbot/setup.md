...
> 先在本地验一次它们是活的：
> ```bash
> node tools/gates/lane-check.test.mjs   # 27 条车道归属断言
> node tools/gates/selfcheck.mjs         # 死链 / 失效 SOP 引用 / 禁用术语
> ```
...
3. 从 `docs/05-grokbot/bot-profiles.md` 复制对应的 **Name / Title / Description** 三段**原文粘贴**
4. 让它做的第一件事：为自己开车道
...
4. 读完这四份文件，然后向我复述你的职责边界（用你自己的话，不要复制粘贴）：
   ...
   - docs/03-process/roles.md 中属于你的那一节
...
对 `docs/05-grokbot/skills/` 下的每一个文件：

1. 在总督的对话里发：
   ```
   请把 /workspace/sunset-club/docs/05-grokbot/skills/sop-xxx.md 的内容
...
详细定义见 `docs/05-grokbot/routines.md`。这里给最小可用集（M0 必须配齐 6 条）：
...
- 任何向外部发送邮件、发推、发消息的动作
- 任何购买、支付、订阅动作
- 任何修改 `docs/00-charter/constitution.md` 或 `vision.md` 的提交
...
2. 为新 Bot 开车道
3. 更新 `docs/03-process/ownership.md` 与 `.github/CODEOWNERS`
