...
### 第三条：数据驱动
- 一切内容（怪物、装备、配方、地下城、技能、文案、数值）以数据文件形式存在，受 JSON Schema 校验，不写死在代码里。
...
- `docs/03-process/ownership.md` 中的所有权表规定每一条路径的**唯一**负责 Bot。
...
- 每个里程碑设一次人类品味评审，用固定 rubric 打分（见 `docs/03-process/gates.md`）。低于阈值的里程碑不放行，无论任务完成率是多少。
...
- 本项目的流程文件（`docs/03-process/`、`docs/05-grokbot/skills/`）是**代码**，同样走 PR 和版本管理。
